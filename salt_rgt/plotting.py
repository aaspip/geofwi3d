    ...
    seis = np.concatenate(
        (
            np.concatenate(
                (
                    0.5*np.ones([1,40]),
                    np.expand_dims(np.linspace(0.5,1,88),axis=1).transpose(),
                    np.expand_dims(np.linspace(1,0,88),axis=1).transpose(),
                    np.zeros([1,40])
                ),
                axis=1
            ).transpose(),
            np.concatenate(
                (
                    0.25*np.ones([1,40]),
                    np.expand_dims(np.linspace(0.25,1,88),axis=1).transpose(),
                    np.expand_dims(np.linspace(1,0,88),axis=1).transpose(),
                    np.zeros([1,40])
                ),
                axis=1
            ...
            np.concatenate(
                (
                    np.zeros([1,40]),
                    np.expand_dims(np.linspace(0,1,88),axis=1).transpose(),
                    np.expand_dims(np.linspace(1,0,88),axis=1).transpose(),
                    np.zeros([1,40])
                ),
                axis=1
...
def plot_f3_data_label(data,label,suptitle=None, save_path=None):
    ...
    titles = ["F3 Seismic Data","F3 Label"]
    ...
    # plot
    # =========================
    for ax, volume, title, cmap_name in zip(axes, data_list, titles, cmaps):
...
def plot_geofwi3d_data_label(data,label,suptitle=None, save_path=None):
    ...
    titles = ["GeoFWI Seismic Data","GeoFWI Salt Label"]
...
def plot_all_models(data_root, model_folders, index, suptitle=None, save_path=None):
    ...
    cmaps = ['gray', 'jet', 'viridis', 'jet', 'jet']
...
def plot_seismic_velocity_salt(data_root, model_folders, index, suptitle=None, save_path=None):
    image, vp, rgt, fault, salt = read_models(data_root, model_folders, index)
    ...
    for ax_i, data_i, title_i, cmap_i in zip(ax, data_list, titles, cmaps):
        ax_i.set_title(title_i, fontsize=16, fontweight="bold", pad=15)
...
def plot_seismic_rgt_salt(data_root, model_folders, index, suptitle=None, save_path=None):
    image, vp, rgt, fault, salt = read_models(data_root, model_folders, index)
